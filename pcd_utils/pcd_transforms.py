...
def random_sample_cluster(point_set, strength):
    """
    Randomly sample the cluster points
    输入: point_set:  最后一位应该是聚类体的下标
          strength:   下采样的强度, 数值越大，强度越小。   Points_sample = Points * strength
    """
    different_label = list(set(point_set[:, -1]))
    sample_points = np.zeros(shape=(1, point_set.shape[1]))
    for idx in range(0, len(different_label)):
        if different_label[idx] == -1:
            points_idx = (point_set[:, -1] == different_label[idx])   # 对应聚类体的点下标
            cluster_points = point_set[points_idx, :]
            sample_points = np.concatenate((sample_points, cluster_points))
        else:
            points_idx = (point_set[:, -1] == different_label[idx])   # 对应聚类体的点下标
            cluster_points = point_set[points_idx, :]
            if cluster_points.shape[0] > 200:
                random_sample_points_idx = np.random.choice(cluster_points.shape[0], max(100, int(cluster_points.shape[0] * strength)), replace=False)   # 如果少于100个点，可能就语义信息太不明确了
                sample_points = np.concatenate((sample_points, cluster_points[random_sample_points_idx, :]))
                corr_points = cluster_points[random_sample_points_idx, :]
            else:
                sample_points = np.concatenate((sample_points, cluster_points))
                corr_points = cluster_points
    sample_points = sample_points[1:, :]

    return sample_points

def down_sample_points(points, strength=0.8):

    strength_rondom = strength + 0.1 * random.random()
    if strength_rondom >= 1:
        strength_rondom = 1
    elif strength_rondom <= 0.5:
        strength_rondom = 0.5
    # print("The strength_rondom is", strength_rondom)

    random_sample_points_idx = np.random.choice(points.shape[0],
                                                max(100, int(points.shape[0] * strength_rondom)),
                                                replace=False)  # 如果少于100个点，可能就语义信息太不明确了
    return points[random_sample_points_idx, :]
...
def rotate_perturbation_point_cloud_XY(batch_data, angle_sigma=0.06, angle_clip=0.18):
...
def random_drop_n_cuboids(batch_data, s=0.5):
    """ Randomly drop N cuboids from the point cloud.
        Input:
            BxNx3 array, original batch of point clouds
            s:随机drop的强度，0.5是默认值，值越大，drop的强度越低，最高取到1
        Return:
            BxNx3 array, dropped batch of point clouds
    """

    index_all = []
    batch_data, index_get = random_drop_point_cloud(batch_data)
    index_all.append(index_get)
    cuboids_count = 1
...
def random_cuboid_point_cloud(batch_data, s=0.25):
    ...
    for batch_index in range(B):
        ...
        crop_range = s + (np.random.rand(2) * 0.5)
        ...
        new_pointidx_count.append(new_pointidx)
    new_batch_data = np.array(new_batch_data)

    return np.squeeze(new_batch_data, axis=0), new_pointidx_count
...
def random_drop_point_cloud(batch_data):
    ...
    for batch_index in range(B):
        ...
        new_batch_data.append(batch_data[batch_index,new_pointidx,:])
        new_pointidx_count.append(new_pointidx)
    return np.array(new_batch_data), new_pointidx_count
