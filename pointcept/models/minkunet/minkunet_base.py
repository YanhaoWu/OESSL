import sys
sys.path.append("/home/WuYanhao/WorkSpace/DepthContrast/models/trunks/spconv/models/modules") 
import os
# print(os.getcwd())
import time
...
class SMLP(nn.Module):
    def __init__(
        ...
        counter = 1
        for dim in dims[1:]:
            layers.append(ME.MinkowskiLinear(last_dim if counter > 1 else 96, dim, bias=use_bias))
            counter += 1
            if use_bn:
                layers.append(
...
class MinkUNet(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.final = conv(cs[8], 20, kernel_size=1, stride=1, bias=True, D=self.D)
...
class MinkUNetSMLP(nn.Module):
    ...
    def forward(self, x, feature_name):
        # feature_name 是 DepthContrast里面整出来的奇怪的东西，可以暂时先不管他
        # x = ME.SparseTensor(features=batch_dict['voxel_features'].cpu(), coordinates=batch_dict['voxel_coords'].cpu(), device=torch.device('cuda'))
        ...
        out_feats = [None]
        feat = self.maxpool(y4)
        feat = self.head(feat)
        out_feats[0] = feat.F ### Just use smlp
